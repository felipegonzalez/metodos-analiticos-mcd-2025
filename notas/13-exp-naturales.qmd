...
del tratamiento son homogéneos sobre los individuos (no hay heterogeneidad en los efectos del tratamiento).
...
- La media de Pureza depende la Compañía, y el efecto es el mismo para
todas las unidades.
- La media de Cólera depende de la pureza, y el efecto es el mismo para todas las unidades.
...
En primer lugar, si los niños deciden por sí mismos subirse o no a la atracción,
es posible que tengamos variables de confusión entre decisión de subirse y el resultado
de malestar. Supondremos que tenemos disponible también la estatura de cada niño,
que creemos que puede influenciar tanto en su decisión de subirse como en el resultado:
...
seleccionar al azar a algunos niños para subirse y otros no. Esto no es factible.
Sin embargo,
...
Entonces hay una variable observable (la estatura)
...
por lo que podemos restringir el análisis a un **intervalo chico** alrededor del
punto de corte 1.20, y para fines prácticos:

- Si restringimos, en el límite lo único que influye en recibir el tratamiento o no es la
estatura (ligeramente por abajo o por arriba de 1.20), y no hay flechas de $U$ tratamiento.

El diagrama se convierte en:
...
1. Diferencias mínimas en estatura determinan el tratamiento (el instrumento es perfecto)
2. Caminos no causales a través de Estatura están
prácticamente bloqueados, pues prácticamente estamos condicionando a un valor de Estatura fijo.

Adicionalmente, supondremos que el efecto del tratamiento es homogéneo en el grupo de niños

Podemos entonces usar diferencias mínimas en la estatura como instrumento para estimar
el efecto causal promedio para los niños del grupo. Como el instrumento
es perfecto, entonces basta comparar los grupos de tratamiento y control directamente. 
Suponiendo que el efecto es homogéneo en los niños, esto es suficiente para tener una estimación
