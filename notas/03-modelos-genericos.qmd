...
de nuestras verificaciones, como veremos más adelante. En general
...
de 30 kilos, por ejemplo. Podríamos poner una inicial como 
...
modelos estadísticos para contestar cada pregunta*, y cada pregunta exigirá
que se cumplan ciertas condicines en nuestro modelo gráfico. El modelo causal
...
de 15 kg, que es lo que esperábamos según el cálculo de arriba. Podemos replicar el cálculo que hicimos arriba directamente usando simulación:
...
hacer condicionando o estratificando por  los valores de $H$ 
(bajo los supuestos de nuestro diagrama). Es decir, para
...
**Nota**: si consideráramos que hay una causa común para estatura y peso,
veremos y explicaremos con detalle más adelante por qué esta 
estrategia de condicionar bajo $H$ **no** funciona para calcular el efecto
directo de sexo sobre peso.

Bajos los supuestos de nuestro modelo original, 
...
un tiro fallido), y el ángulo $\theta$ de salida. Adicionalmente, hay otros factores
...
en la distancia, esperaríamos obtener tasas de éxito diferentes. Igualmente, 
podemos poner una flecha de $V$ a $D$ y $V$ a $Y$: son factores no medidos que
afectan la probabilidad de éxito, y que pueden variar por la distancia.
